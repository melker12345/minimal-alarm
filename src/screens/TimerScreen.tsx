...
export function TimerScreen({timer}: {timer: TimerController}) {
  ...
  const {durationSec, remaining, phase, endsAt, setDuration, start, pause, reset, addMinute} = timer;
    ...
    return (
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.presets}>
          {PRESETS.map(value => {
            const active = durationSec === value * 60;
            return (
              <Tappable key={value} onPress={() => pick(0, value, 0)} frame={[styles.preset, active && styles.presetActive]} style={styles.presetInner}>
